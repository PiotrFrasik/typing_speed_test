...
class TypingSpeedTest:
    ...
    def __init__(self):
        ...
        self.options_pad = self.title_pad = None
    ...
    def title_blink(self, stdscr):
        ...
        while True:
            self.title_pad.addstr(0, 0, "<------ Welcome in typing speed test! ------>", color_title)
            self.title_pad.refresh(0, 0, 0, 0, 0, 50)

            self.options_pad.addstr(0, 0, f"Choose the options:")
            self.options_pad.addstr(1, 0, f"-> WPM in live click key 1")
            self.options_pad.addstr(2, 0, f"-> Typing in time click key 2")
            self.options_pad.refresh(0, 0, 2, 0, 6, 50)

            # change_title
            if time.time() - start_time > 0.7:
                self.title_pad.clear()
                self.title_pad.refresh(0, 0, 1, 0, 4, 50)

                time.sleep(0.5) #pause in show on the screen
